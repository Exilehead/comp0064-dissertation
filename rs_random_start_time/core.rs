...
impl Core {
    ...
    async fn make_vote(&mut self, block: &Block, random: u32) -> Option<Vote> {
        let nanos = random;
    ...
    #[async_recursion]
    async fn handle_vote(&mut self, vote: &Vote, random: u32) -> ConsensusResult<()> {
    ...
    #[async_recursion]
    async fn process_block(&mut self, block: &Block, random: u32) -> ConsensusResult<()> {
        ...
        if let Some(vote) = self.make_vote(block, nanos).await {
            debug!("Created {:?}", vote);
            let next_leader = self.leader_elector.get_leader(self.round + 1);
            if next_leader == self.name {
                self.handle_vote(&vote, nanos).await?;
            } else {
    ...
    async fn handle_proposal(&mut self, block: &Block, random: u32) -> ConsensusResult<()> {
        ...
        if (block.round == 15 || block.round == 1 ) && (nanos % 1000 == 500 || nanos % 1000 == 600) && block.author != self.leader_elector.get_leader(block.round) {
            info!("PYTHON DETECTOR CANNOT DETECT FROM NOW!");
            info!("Attack 4 potentially detected!");
            info!("But I'm MALICIOUS NODE {} and won't tell you anything!", self.name);
            info!("Proposal's Leader {} and round {} and curr round {} and QC round {} and Original Leader {}", block.author, block.round, self.round, block.qc.round, self.leader_elector.get_leader(block.round));
        ...
        self.process_block(block, nanos).await
    }
        ...
        loop {
            let result = tokio::select! {
                Some(message) = self.rx_message.recv() => match message {
                    ConsensusMessage::Propose(block) => self.handle_proposal(&block, random).await,
                    ConsensusMessage::Vote(vote) => self.handle_vote(&vote, random).await,
                    ConsensusMessage::Timeout(timeout) => self.handle_timeout(&timeout).await,
                    ConsensusMessage::TC(tc) => self.handle_tc(tc).await,
                    _ => panic!("Unexpected protocol message")
                },
                Some(block) = self.rx_loopback.recv() => self.process_block(&block, random).await,
